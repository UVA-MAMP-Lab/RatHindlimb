...
def mirror_geometry(geometry: osim.Geometry, axes: list) -> osim.Geometry:
    ...
    new_scale_factors = osim.Vec3(
        mirrored_values[0], mirrored_values[1], mirrored_values[2]
    )
...
def mirror_body(body: osim.Body, axes: list) -> osim.Body:
    ...
    try:
        ...
        print(
            f"    Caught RuntimeError accessing index 0. Assuming no geometry attached or error. Details: {e}"
        )
    except Exception as e:
        # Catch any other Python-level exceptions that might occur
        print(
            f"    Caught unexpected Python exception accessing index 0: {type(e).__name__} - {e}"
        )
    ...
    for i in range(wrap_objects.getSize()):
        ...
        xyz_body_rotation = osim.Vec3(
            [
                -xyz_body_rotation[i] if i not in axes else xyz_body_rotation[i]
                for i in range(3)
            ]
        )
        ...
        # Negate the appropriate axes based on the axes
        translation = osim.Vec3(
            [-translation[i] if i in axes else translation[i] for i in range(3)]
        )
...
def mirror_joint(joint: osim.Joint, axes: list) -> osim.Joint:
        ...
        case "CustomJoint":
            ...
            for i, vec in enumerate(transform_axes):
                ...
                get_transform_function = (
                    ("get_rotation" + str(i + 1))
                    if i < 3
                    else ("get_translation" + str(i - 2))
                )
                set_transform_function = (
                    ("set_rotation" + str(i + 1))
                    if i < 3
                    else ("set_translation" + str(i - 2))
                )
                transform_axis: osim.TransformAxis = getattr(
                    spatial_transform, get_transform_function
                )()
                transform_coords: osim.ArrayStr = (
                    transform_axis.getCoordinateNamesInArray()
                )
                ...
                if any(vec[axes]):
                    ...
                    match concreteClass:
                        case "SimmSpline":
                            # Mirror the function
                            simm_spline: osim.SimmSpline = osim.SimmSpline.safeDownCast(
                                axis_function
                            )
                        ...
                        case "LinearFunction":
                            linear_func: osim.LinearFunction = (
                                osim.LinearFunction.safeDownCast(axis_function)
                            )
                        ...
                        case "Constant":
                            constant_func: osim.Constant = osim.Constant.safeDownCast(
                                axis_function
                            )
                        ...
                        case "MultiplierFunction":
                            mult_func: osim.MultiplierFunction = (
                                osim.MultiplierFunction.safeDownCast(axis_function)
                            )
                            if mult_func is None:
                                raise ValueError(
                                    "Function is not a MultiplierFunction."
                                )
                            # For MultiplierFunction, negate the scale
                            mult_func.setScale(-mult_func.getScale())
                        case _:
                            print(
                                f"Unsupported function type: {concreteClass}, not mirroring"
                            )
    ...
    while True:
        ...
        orientation = osim.Vec3(
            [-orientation[i] if i not in axes else orientation[i] for i in range(3)]
        )
...
def mirror_model(
    ...
    muscle_name_mapping: dict[str, str] = {r"R_(.+)": r"L_\1"},
) -> osim.Model:
    """
    ...
    for joint_name, bodies in model.joint_bodies.items():
        if exclude_bodies and (
            bodies[0] in exclude_bodies and bodies[1] in exclude_bodies
        ):
