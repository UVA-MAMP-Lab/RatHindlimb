...
import numpy as np
import re
from src.processing import update_model
...
The original model uses the Thelen [@missing-reference] muscle model, which have been largely superseded by the Millard [@missing-reference] muscle model in OpenSim. We can convert the muscles using the [`model_thelen_to_millard`](src/rathindlimb/muscle_utils.py) function. 
...
from src.plotting import muscle_length_parcoords
...
```{python}
from src.registration import register_meshes, apply_transformation_to_mesh
