...
First, we update naming conventions to signify that the unilateral model is the right side. We also replace the .vtp meshes with .stl files for more modern support.
...
The original model uses the Thelen [@thelenAdjustmentMuscleMechanics2003] muscle model, which has been largely superseded by the Millard [@millarmillardFlexingComputationalMuscle2013] muscle model in OpenSim especially for muscle-level analysis. We can convert the muscles using the [`model_thelen_to_millard`](src/rathindlimb/muscle_utils.py) function.

```{python}
from src.rathindlimb.muscle_utils import model_thelen_to_millard
model = osim.Model(str(model_file))
...
We lock sacroiliac flexion, ankle internal rotation, and ankle adduction as we do not have the resolution to capture the small amount of motion that these correspond to, and they add unnecessary complexity to the model for the analysis we intend to perform.
...
young_attachments = pd.read_csv(attachment_dir / 'young_model_attachments.csv')

md(tabulate(young_attachments, headers='keys', tablefmt='pipe'))
```
...
Our model uses a slightly different marker set than the one used in Johnson's which is detailed in a previous work [@jackdienesComprehensiveDynamicKinematic2022]
...
The original Johnson model from 2008 had estimated parameters. In a subsequent paper in 2011 [@johnsonApplicationRatHindlimb2011], @tbl-johnson-params
Another paper by Eng [@Eng2008] also provided muscle parameters for the rat hindlimb, shown in @tbl-eng-params. A comparison of the two sets of parameters is shown below.
...
Two ranges of motion are used to estimate the optimal fiber length and tendon slack length of muscles based on observed fiber lengths during motion. The first is the model's full range of motion, and the second is the observed range of motion during walking. For these ranges, the normalized fiber length ranges were set to (0.5, 1.5) for the full ROM and (0.6, 1.2) for walking. These determine the bounds within which the optimization will search for the optimal fiber length, thus the optimization is heavily dependent on the selected ranges. Absent any literature confirming the ranges for rat hindlimb muscles, these values were chosen based on vibes? 

{{< embed notebooks/tsl_optimization.ipynb#tbl-tsl-comparison >}}

Any tendon slack lengths estimated as 0 are set to rigid.
```{python}
tsl_df = pl.read_csv(data_dir / 'parameters' /'tsl_comparison.csv')

muscles : osim.SetMuscles = model.getMuscles()
for i in range(muscles.getSize()):
    muscle : osim.Muscle = muscles.get(i)
    muscle_name = muscle.getName().replace('R_', '')
    params = tsl_df.row(by_predicate=pl.col('Abbreviation') == muscle_name, named=True)
    if params is None:
        print(f"Muscle {muscle_name} not found in parameters file.")
        continue
    lts = params['Walk TSL (mm)'] / 1000  # Convert mm to m
    if lts <= 0:
        muscle.set_ignore_tendon_compliance(True)
    muscle.setTendonSlackLength(lts)
    print(f"Updated {muscle_name}: lts={lts}")
